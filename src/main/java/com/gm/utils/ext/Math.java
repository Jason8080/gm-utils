 ...
 * @author Jason
 */
public class Math implements Utils {
    ...
    public static <T> T execute(String expression, Class<T> tClass) {
        Assert.isTrue(classes.contains(tClass), "暂不支持" + tClass + "类型");
    ...
    private static <T> T add(T t1, T t2) {
        ...
        throw new BusinessException("不支持" + t1.getClass() + "类型的\"+\"运算");
    }
    ...
    private static void logicOperator(Stack<Boolean> resultStack, Stack<String> logicStack) {
        ...
        if (op.equals(NOT)) {
            ...
            throw new BusinessException("不支持" + op + "逻辑符");
        }
    }
    ...
    private static boolean compare(Stack<BigDecimal> operandStack, Stack<String> operatorStack) {
        ...
        throw new BusinessException("不支持" + op + "比较符");
    }
    ...
    public static Class getClass(String expression){
        Str str = new Str(expression);
        //逻辑运算用Bool
        if (str.contains(logic.toArray(new String[]{}))){
