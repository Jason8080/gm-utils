...
- 模块总览  
  ...
  ![GM-TOOLS模块分布图](http://assets.processon.com/chart_image/5f83d799e401fd06fd82d8c4.png)
...
- 日志持久化: **方式1**
  ...
  - 重写: 需要按自己的表、格式、内容保存到数据库
    ...
    1. 实现cn.gmlee.tools.logback.db.Log实体
    ...
    3. 在resources/mapper下创建{ DtoMapper.xml }
       ...
       <!-- Log: 默认实体 -->
       <mapper namespace="cn.gmlee.tools.logback.db.Log">
...
- 日志持久化: **方式2**
  ...
  - 效果: [2020-12-03 18:05:40.538] [http-nio-9740-exec-1] [INFO ] c.huolala.common.logback.config.aop.ApiPrintAspect - 
    ...
    ```json
    {
    	...
    	"总耗时":"2",
    	"代码位置":"93:cn.huolala.nacos.server.pos.order.controller.TxController.gray"
    }
    ```
  ...
  - **Redis输出**: 将日志输出到Redis
    ...
    ```xml
    <appender name="REDIS" class="cn.huolala.common.logback.appender.RedisAppender">
